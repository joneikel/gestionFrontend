...
const MapActivities = ({ muncipalityCode, checkedIntitutions, filters }: MapActivitiesProps) => {
  ...
  return (
    <>
      {loading && <Spin size="large" className="loading-map-activities-spinner" />}
      {activities
        .filter(
          (act) =>
            act.lat &&
        ...
        .map((act, i) => {
          return (
            ...
            act.lng && (
              <Marker
                icon={makeMarker({ institution: act.project.program.institution })}
